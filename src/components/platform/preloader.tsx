...
export function Preloader({ onComplete, show = true }: PreloaderProps) {
  const [quote, setQuote] = useState<PreloaderQuote | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchQuote = async () => {
      try {
        ...
        setQuote({ id: 'fallback', quote: 'Education is the passport to the future.', author: 'Malcolm X' });
      }
    };
  ...
  return (
    ...
    >
      ...
      {!quote && (
        <p className="text-sm text-gray-400 font-medium animate-pulse">Loading your classroom... ✨</p>
      )}
